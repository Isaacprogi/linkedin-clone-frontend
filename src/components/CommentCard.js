        ...
        <div className={`w-full flex ${switchTheme ? "bg-gray-600" : ""} flex-col p-2 rounded-b-lg rounded-r-lg rounded-rb-lg bak`}>
          <div className={`w-full flex items-center justify-between`}>
            ...
            <span className='text-[.95rem] flex items-start leading-[.9rem]' >
              ...
              <span>
                <div className={` text-[.75rem] ${switchTheme ? "text-gray-300" : ""} font-[600]`}>{comment?.user?.firstname + " " + comment?.user?.lastname}</div>
                <span className={`text-[.75rem] ${switchTheme ? "text-gray-400" : ""} font-mono`}>{comment?.user?.title}</span>
              </span>
              {
                comment?.user?._id === feed?.user?._id ? <span className='text-white  px-1 ml-2 h-[1.2rem] flex items-center justify-center rounded-md bg-blue-900  '>
                  <span className="mt-[.23rem]">
                    Author
                  </span>
                </span> :
                  ''
            ...
            <span className='flex items-center text-[.9rem] text-gray-500  '>
              ...
              <span className='relative'>
                ...
                {
                  deleting === true ? <button disabled={commentDeleteLoading} onClick={handleCommentDelete} className="w-[4rem] text-grray-600 hover:bg-blue-100 hover:text-gray-500 shadow-lg cursor-pointer absolute top-[2rem] border border-gray-300 right-[1rem] rounded-md flex items-center justify-center h-[max-content] bg-white ">
                    Delete
                  </button> : ""
                }
              </span>
          ...
          <div className={`w-full text-[.75rem] ${switchTheme ? "text-gray-300" : "text-gray-600"} mt-[.5rem] text-[.9rem]`}>{comment?.comment}</div>
        </div>
